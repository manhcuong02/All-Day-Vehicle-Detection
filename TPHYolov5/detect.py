...
from TPHYolov5.utils.plots import Annotator, colors
from TPHYolov5.utils.torch_utils import load_classifier, select_device, time_sync

def display_results(img, results):
    
    rate = max(img.shape[:2])/960
    
    expand = np.ones((img.shape[0], int(150*rate), 3), dtype = np.uint8)*255
    
    new_img = np.concatenate([img, expand], axis = 1)
    ...
    for key, value in results.items():
        ...
        cv2.putText(new_img, f"{key}: {value}", (img.shape[1] + int(10*rate), int(space*count)), cv2.FONT_HERSHEY_SIMPLEX, 0.5*rate, (0,0,255), thickness = thickness)
        count += 1       
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, img, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            results["FPS"] = round(1/(t3 - t2)*vid_stride, 2)
            im0 = display_results(im0, results)
